...
//                     col++;
//                 }
//                 return { RELOP, relop, line, start_col };
//             }
//             default:
//                 pos++;
//                 col++;
...
//             node->children.push_back(Stmt(out));
//         }
//         // 如果当前token既不是分号也不是end，检查是否是最后一个语句
//         if (currentToken.type != END && currentToken.type != SEMICOLON) {
//             // 保存当前位置
//             size_t savedPos = lexer.getPos();
//             int savedLine = lexer.getLine();
//             int savedCol = lexer.getCol();
            
//             // 尝试读取下一个token
//             Token nextToken = lexer.getNextToken();
            
//             // 如果下一个token是end，说明这是最后一个语句，不需要分号
//             if (nextToken.type == END) {
//                 // 恢复位置
//                 lexer.setPos(savedPos);
//                 lexer.setLine(savedLine);
//                 lexer.setCol(savedCol);
//                 currentToken = nextToken;
//             } else {
//                 error("语句之间必须用分号分隔", out);
//             }
//         }
//         return node;
//     }
...
//         node->children.push_back(new ASTNode{ currentToken.value });
//         eat(ID, out);
//         eat(ASSIGN, out);
...
//             auto notNode = new ASTNode{ "逻辑符: " + currentToken.value };
//             eat(NOT, out);
